...
ex::test::Small::Small(const uint32_t* _data, const uint32_t* _end) {
...
protocache::Data ex::test::Small::Serialize() const {
	using _ = ::test::Small::_;
	std::vector<protocache::Data> parts(3);
	if (i32 != 0) { parts[_::i32] = protocache::Serialize(i32); }
	if (flag) { parts[_::flag] = protocache::Serialize(flag); }
	if (!str.empty()) {
...
ex::test::Main::Main(const uint32_t* _data, const uint32_t* _end) {
	using _ = ::test::Main::_;
	protocache::Message _view(_data, _end);
	protocache::ExtractField(_view, _::i32, _end, &i32);
...
protocache::Data ex::test::Main::Serialize() const {
	using _ = ::test::Main::_;
	std::vector<protocache::Data> parts(30);
	if (i32 != 0) { parts[_::i32] = protocache::Serialize(i32); }
	if (u32 != 0) { parts[_::u32] = protocache::Serialize(u32); }
	if (i64 != 0) { parts[_::i64] = protocache::Serialize(i64); }
	if (u64 != 0) { parts[_::u64] = protocache::Serialize(u64); }
	if (flag) { parts[_::flag] = protocache::Serialize(flag); }
	if (mode != 0) { parts[_::mode] = protocache::Serialize(mode); }
	if (!str.empty()) {
		parts[_::str] = protocache::Serialize(str);
		if (parts[_::str].empty()) return {};
	}
	...
	if (f32 != 0) { parts[_::f32] = protocache::Serialize(f32); }
	if (f64 != 0) { parts[_::f64] = protocache::Serialize(f64); }
	parts[_::object] = protocache::Serialize(object);
	if (parts[_::object].empty()) return {};
	if (parts[_::object].size() == 1) { parts[_::object].clear(); }
	if (!i32v.empty()) {
	...
	if (t_u32 != 0) { parts[_::t_u32] = protocache::Serialize(t_u32); }
	if (t_i32 != 0) { parts[_::t_i32] = protocache::Serialize(t_i32); }
	if (t_s32 != 0) { parts[_::t_s32] = protocache::Serialize(t_s32); }
	if (t_u64 != 0) { parts[_::t_u64] = protocache::Serialize(t_u64); }
	if (t_i64 != 0) { parts[_::t_i64] = protocache::Serialize(t_i64); }
	if (t_s64 != 0) { parts[_::t_s64] = protocache::Serialize(t_s64); }
	if (!index.empty()) {
	...
	if (parts[_::matrix].size() == 1) { parts[_::matrix].clear(); }
	if (!vector.empty()) {
	...
	if (parts[_::arrays].size() == 1) { parts[_::arrays].clear(); }
	return protocache::SerializeMessage(parts);
}
